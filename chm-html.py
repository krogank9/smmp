	...
	for child in cur.getNodeChildren():
		...
		for param in obj.getNodeChildren():
			if not first_name_param and param.attrsObj["name"] == "Name":
				...
				index_links.append(param.attrsObj["value"].replace("\\","/"))
	...
	for li in lis:
		...
		for param in obj.getNodeChildren():
			...
			if param.attrsObj["name"] == "Local":
				li_arr[1] = ensurePathIsFile(param.attrsObj["value"].replace("\\","/"))
...
#######################################################
# Now put it all together and output the CHM browser: #
#######################################################

#open chm source from user input
...
#all done. output the new html file with the CHM source packaged in
os.chdir(original_cwd)
codecs.open("gen_chm.html", "w", "utf-8-sig").write(CHM_TEMPLATE)


############################################
# todo support more charsets somehow...    #
# currently just tries utf-8 then chinese. #
############################################
