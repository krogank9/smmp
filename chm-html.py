...
for var, word in zip(replace_vars, replace_words):
	replacing = word
	pre_len = len(gen_chm)
	pre_find = gen_chm.find(replacing) + len(replacing)
	gen_chm = replaceAfterIndex(gen_chm, replacing, str(var), 0)
	next_index = pre_find + (len(gen_chm) - pre_len)

codecs.open("gen_chm.html", "w", "utf-8-sig").write(gen_chm)
